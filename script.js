  ...
  for (let semestre = 1; semestre <= 8; semestre++) {
    ...
    materias
      ...
      .forEach(m => {
        ...
        const requisitos = m.prereq || [];
        const desbloqueada = requisitos.every(id => aprobadas.has(id));
        if (!desbloqueada) matDiv.classList.add("locked");
        if (aprobadas.has(m.id)) matDiv.classList.add("completed");
